        ...
        teamsIds.map(async (teamObj) => {
          try {
            ...
            if (superUsersArr.length > 0) {
              selectQuery = `SELECT distinct A.user_id,A.user_aadobject_id,A.LANGUAGE_ID,B.SOS_NOTIFICATION,B.INTEGRATION_CONFIGURE,B.SHOW_SOS_BUTTON as ALLOW_USERS_TO_SEND_SOS_REQUEST,B.created_date,B.FOLLOW_UP_INCIDENT_NOTIFICATION,B.SEND_INCIDENT_MESSAGES_VIA,B.PHONE_FIELD, B.serviceUrl, B.user_tenant_id, A.user_name, B.team_id, B.team_name,B.IS_APP_PERMISSION_GRANTED,B.SEND_EMAIL
...
SELECT
    ...
    u.user_aadobject_id,
    a.status,
    (select top 1 user_name from MSTeamsTeamsUsers where user_aadobject_id=a.FIRST_RESPONDER) as first_responder,
	a.FIRST_RESPONDER_RESPONDED_AT as responded_at,
    a.closed_by_user,
...
WHERE rn = 1
  AND (
        user_aadobject_id = '${userid}'
        OR EXISTS (
            SELECT 1
            FROM MSTeamsInstallationDetails mid
            WHERE mid.uninstallation_date IS NULL
              AND EXISTS (
                    SELECT 1
                    FROM STRING_SPLIT(B.team_ids, ',', 1) t
                    WHERE LTRIM(RTRIM(t.value)) = mid.team_id
              )
              AND (
                    mid.user_obj_id = '${userid}'
                    OR (',' + mid.super_users + ',' LIKE '%,' + '${userid}' + ',%')
              )
        )
    )
...
const addMembersIntoIncData = async (
  ...
  try {
    ...
    for (let i = 0; i < allMembers.length; i++) {
      ...
      let memberid = member.id || member.value;
...
const getRecurringOccurrencesBatch = async (
  ...
  try {
    ...
    uniqueIds.forEach((incId) => {
      ...
      const incStatusId = statusByInc[incId] != null ? statusByInc[incId] : -1;
      const incidentClosed = incStatusId === 2;
      const occurrences = runAtList.map((runAt) => {
        const isLatest = areRunAtEqual(runAt, lastRunAt);
        const status = isLatest && !incidentClosed ? "Ongoing" : "Closed";
...
const getIncByOccurrenceRunAt = async (incId, runAt, userAadObjId = null) => {
  try {
    ...
    processSafetyBotError(
      ...
      "",
      userAadObjId,
      "error in getIncByOccurrenceRunAt incId=" + incId + " runAt=" + runAt,
    );
    return null;
  }
};
...
const getIncStatus = async (incId, runAt = null) => {
  let incStatusId = -1;
  const hasRunAt =
    runAt != null && runAt !== "" && runAt !== "undefined" && runAt !== "null";
...
const getUserTeamInfo = async (userAadObjId) => {
  ...
  try {
    const sqlTeamInfo = `SELECT  
     ...
     created_date,
     SHOW_SOS_BUTTON as ALLOW_USERS_TO_SEND_SOS_REQUEST,
...
const getManualLocations = async (tenantId) => {
  ...
  const query = `
    ...
    WHERE TENENT_ID = @tenantId
      AND (ISOffice365Location IS NULL OR ISOffice365Location = 0)
    ORDER BY COUNTRY, CITY
  `;
...
const saveManualLocations = async (body) => {
  ...
  try {
    ...
    for (const location of locations) {
      ...
      saveRequest.input("state", sql.NVarChar(sql.MAX), location.state || null);
      saveRequest.input(
        "department",
...
const normalizeLocationKeyPart = (value) => String(value ?? "").trim();
...
const getUserLanguageIdByAadObjId = async (userAadObjId) => {
  ...
  const DEFAULT_LANGUAGE_ID = attributeTranslationService.DEFAULT_LANGUAGE_ID;
  try {
    if (!userAadObjId) {
...
const getIncDataToCopyInc = async (
  ...
  try {
    ...
    const sqlWhereResponseMembers = ` u.team_id = '${teamId}' and u.user_aadobject_id in (select user_id from MSTeamsIncResponseSelectedUsers where inc_id = ${incId})`;
    const sqlResponseMembers =
      ` SELECT distinct u.[USER_ID] [value] , u.[USER_NAME] [title], u.user_aadobject_id userAadObjId, 0 isSuperUser,0 iscreateIncidentUser` +
      `, u.conversationId,
    u.DYNAMIC_LOCATION,
    u.LANGUAGE_ID,
  case when inst.user_id is null then 0 else 1 end isAdmin , city, country, state, department,u.email,u.hasLicense
  FROM MSTEAMSTEAMSUSERS u
  left join MSTeamsInstallationDetails inst on u.user_id = inst.user_id and u.team_id = inst.team_id and inst.uninstallation_date is null ` +
      ` WHERE ${sqlWhereResponseMembers} and u.hasLicense = 1 
ORDER BY u.[USER_NAME]; `;
...
const updateSentSMSCount = async (tenant_id, counter) => {
  try {
