  ...
  const sendProactiveMessage = async (sqlQuery, subcriptionMessage) => {
    ...
    try {
        ...
        await Promise.all(
          jobsToBeExecutedArr.map(async (job) => {
            try {
              ...
              let card = getTypeThreeSubscriptionStartedCard(
                memberCount,
                subscriptionDate,
                expiryDate,
                userObj
              ...
              await incidentService.updateSubcriptionProcessFlag(
                job.ID,
                job.user_aadobject_id
              );
              saveLog = true;
              ...
              processSafetyBotError(
                ...
                "",
                job.user_aadobject_id,
                "error in sendProactiveMessage subscription ID-" + job.ID
              );
            }
          })
        );
      ...
      processSafetyBotError(
        ...
        "",
        "",
        "error in sendProactiveMessage newSubscriptionAdded Job jobsToBeExecutedArr=" +
