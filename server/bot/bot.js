  ...
  for (const user of usrPhones) {
    ...
    try {
      ...
      const call = await tClient.calls.create({
        twiml: `
<Response>
  <Gather numDigits="1" timeout="8" action="${process.env.serviceUrl}/voicecall?incidentId=${incObj.incId}&userId=${encodeURIComponent(
          user.id,
        )}" method="POST">
    <Say voice="alice">
...
`,
        to: phone,
        from: "+18023277232",
        statusCallback: `${process.env.serviceUrl}/callstatus?incidentId=${incObj.incId}&userId=${encodeURIComponent(
          user.id,
        )}`,
